...
Vector3 Trfm3D::transformPoint(const Vector3 & P) const {
	//construir la matriz de CSR y multiplicar con el punto
	//VECTOR: 	(X Y Z 1)
	//MATRIZ A CONSTRUIR:
	...
	/*     |   d.x     d.y     d.z   w   | */
	float v [4];	v[0] = P[0]; v[1] = P[1]; v[2] = P[2]; v[3] = 1.0f;
	float mat [16]; 
	mat[0] = m_c1[0]; mat[1] = m_c2[0]; mat[2] = m_c2[0]; mat[3] = m_tr[0];
	mat[4] = m_c1[1]; mat[5] = m_c2[1]; mat[6] = m_c2[1]; mat[7] = m_tr[1];
	mat[8] = m_c1[2]; mat[9] = m_c2[2]; mat[10] = m_c2[2]; mat[11] = m_tr[2];
	mat[12] = newDX; mat[13] = newDY; mat[14] = newDZ; mat[15] = newDW;
	Vector3 res;
	//puner en res el resultado de la multiplicación
	return res;
}
...
Vector3 Trfm3D::transformVector(const Vector3 & V) const {
	//RECUERDA: LOS VECTORES NO SE TRASLADAN!!!
	//matriz: 	x x 0
	//			y y 0
	//			z z 0
	//VECTOR: (x y z 0)
